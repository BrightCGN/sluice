...
| Gateways scheitern, Kern läuft (oder umgekehrt) | Die Gateway-User sind weder Eigentümer noch Gruppe von `/opt/sluice` — sie kommen nur über `o+rX` an venv und Code. Ein zu enges `chmod` trifft sie zuerst | `sudo -u sluice-gw-anthropic test -x /opt/sluice/.venv/bin/uvicorn`. Reparatur wie in der `203/EXEC`-Zeile unten |
