...
def test_detects_email_ip_mac() -> None:
    d = Detector([])
    text = "Mail an max.mustermann@example.com, Gerät 192.0.2.21 (AA:BB:CC:DD:EE:FF)"
    types = {s.type for s in d.spans(text)}
    assert types == {"EMAIL", "IP", "MAC"}

def test_dictionary_case_insensitive_and_longest_first() -> None:
    d = Detector(["Mustermann", "Musterstraße 12", "Musterstraße"])
    spans = d.spans("mustermann wohnt in der Musterstraße 12.")
...
async def test_forward_reverse_roundtrip() -> None:
    s = _mode(["Mustermann"])
    scope = Scope("test")
    text = "Mustermann (max.mustermann@example.com) meldet 192.0.2.21 offline."
    fwd = (await s.forward(EgressPayload(raw_text=text), scope)).text
    assert "Mustermann" not in fwd
    assert "example.com" not in fwd
...
async def test_pseudonyms_stable_within_scope() -> None:
    s = _mode(["Mustermann"])
    scope = Scope("test")
    a = (await s.forward(EgressPayload(raw_text="Mustermann hier."), scope)).text
    b = (await s.forward(EgressPayload(raw_text="Nochmal Mustermann."), scope)).text
...
async def test_forward_messages_only_touches_content() -> None:
    ...
    sanitized = await s.forward(
        EgressPayload(raw_text="", messages=[{"role": "user", "content": "Ich bin Mustermann"}]),
        Scope("test"),
    )
    assert sanitized.messages[0]["role"] == "user"
    assert "Mustermann" not in sanitized.messages[0]["content"]
...
async def test_reverse_obj_recurses_tool_args() -> None:
    ...
    assert real["nested"]["cc"] == ["max.mustermann@example.com"]
    assert real["n"] == 3
...
async def test_stream_reverser_token_split_across_chunks() -> None:
    s = _mode(["Mustermann"])
    scope = Scope("test")
    await s.forward(EgressPayload(raw_text="Mustermann"), scope)  # erzeugt ⟦NAME_1⟧
    r = s.stream_reverser(scope)
    out = r.feed("Hallo ⟦NA") + r.feed("ME_1⟧, wie geht's?") + r.flush()
    assert out == "Hallo Mustermann, wie geht's?"


async def test_stream_reverser_flush_trailing_token() -> None:
    s = _mode(["Mustermann"])
    scope = Scope("test")
    await s.forward(EgressPayload(raw_text="Mustermann"), scope)
    r = s.stream_reverser(scope)
    out = r.feed("Gruß an ⟦NAME_1⟧") + r.flush()
    assert out == "Gruß an Mustermann"
...
async def test_scopes_share_no_mapping() -> None:
    ...
    fwd_a = (await s.forward(EgressPayload(raw_text="Mustermann meldet sich"), a)).text
    token = "⟦NAME_1⟧"
    ...
    fwd_b = (await s.forward(EgressPayload(raw_text="Anruf von Mustermann"), b)).text
    assert "Mustermann" not in fwd_b
    assert await s.reverse_text(fwd_b, b) == "Anruf von Mustermann"
    assert await s.reverse_text(fwd_a, a) == "Mustermann meldet sich"
...
async def test_mapping_discarded_after_ttl() -> None:
    now = [0.0]
    s = _mode(["Mustermann"], ttl_seconds=3600, clock=lambda: now[0])
    scope = Scope("session")
    fwd = (await s.forward(EgressPayload(raw_text="Mustermann"), scope)).text
